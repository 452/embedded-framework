...
//    private static final int BME280_SOFTRESET_VALUE = 0xB6;
    ...
    public BME280(Config config) throws IOException {
        this.config = config;
        System.out.println("BME280 ID byte: 0x" + Integer.toHexString(read(BME280_REG_ID)));
    }
...
//          Normal mode, temp and pressure over sampling rate = 1
     ...
     */
    public void setTemperature(double themperature) {
    ...
    public interface Config {
        ...
        default void read(int address, byte[] buffer) throws Exception {
            read(address, buffer, buffer.length);
        }
